        ...
        public static int ReadISO9115Metadata()
        {
            ...
            if (files.Length > 0)
            {
                for (int n = 0; n < files.Length; n++)
                {
                        ...
                        if (xmlReader.NodeType == XmlNodeType.Element)
                        {
                            ...
                            switch (xmlReader.Name)
                            {
                                ...
                                case "gmd:MD_DimensionNameTypeCode":
                                    if (xmlReader.Name == "gmd:MD_DimensionNameTypeCode")
                                    {
                                        currentDimension = xmlReader.GetAttribute("codeListValue");
                                    }
                                    break;

                                case "gmd:dimensionSize":
                                case "gco:Integer":
                                    if (xmlReader.Name == "gco:Integer")
                                    {
                    ...
                    var erd = new ERDDAPMetadataReadEventArgs(dataTitle, dataAbstract, beginPosition, endPosition, identifier, url, credit, temporalSize, new Dictionary<string, (string unit, string description)>(_parameters));
