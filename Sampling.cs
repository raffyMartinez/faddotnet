    ...
    public class sampling
    {
        ...
        public static Dictionary<string, string[]> LFData(string CatchCompRowNo)
        {
            ...
            using (var conection = new OleDbConnection("Provider=Microsoft.JET.OLEDB.4.0;data source=" + global.mdbPath))
            {
                ...
                {
                    conection.Open();
                    string query = "SELECT RowGUID, LenClass, Freq FROM tblLF WHERE tblLF.CatchCompRow='{" + CatchCompRowNo + "}' ORDER BY LenClass";
                    var adapter = new OleDbDataAdapter(query, conection);
                    adapter.Fill(dt);
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
        ...
        public static bool UpdateLF(bool isNew, double LenClass, long ClassCount, string CatchCompRow, string RowGUID)
        {
            bool Success = false;
            string query = "";
            using (OleDbConnection conn = new OleDbConnection(global.ConnectionString))
            {
                try
                {
                    if (isNew)
                    {
                        query = "Insert into tblLF (LenClass, Freq, CatchCompRow, RowGUID) values (" +
                            LenClass + ", " + ClassCount + ", '{" + CatchCompRow + "}', '{" + RowGUID + "}')";
                    }
                    else
                    {
                        query = "Update tblLF " +
